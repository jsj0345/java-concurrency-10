...
```text
17:09:05.336 [     main] == [생산자 먼저 실행] 시작, BoundedQueueV3 ==

17:09:05.342 [     main] 생산자 시작
17:09:05.357 [producer1] [생산 시도] data1 -> []
17:09:05.361 [producer1] [put] 생산자 데이터 저장, notify() 호출
17:09:05.361 [producer1] [생산 완료] data1 -> [data1]
17:09:05.468 [producer2] [생산 시도] data2 -> [data1]
17:09:05.468 [producer2] [put] 생산자 데이터 저장, notify() 호출
17:09:05.468 [producer2] [생산 완료] data2 -> [data1, data2]
17:09:05.579 [producer3] [생산 시도] data3 -> [data1, data2]
17:09:05.579 [producer3] [put] 큐가 가득 참, 생산자 대기

17:09:05.689 [     main] 현재 상태 출력, 큐 데이터 : [data1, data2]
17:09:05.689 [     main] producer1: TERMINATED
17:09:05.689 [     main] producer2: TERMINATED
17:09:05.691 [     main] producer3: WAITING

17:09:05.691 [     main] 소비자 시작
17:09:05.693 [consumer1] [소비 시도]  ? <- [data1, data2]
17:09:05.693 [consumer1] [take] 소비자 데이터 획득, notify() 호출
17:09:05.695 [producer3] [put] 생산자 깨어남
17:09:05.695 [producer3] [put] 생산자 데이터 저장, notify() 호출
17:09:05.695 [consumer1] [소비 완료] data1 <- [data2]
17:09:05.695 [producer3] [생산 완료] data3 -> [data2, data3]
17:09:05.800 [consumer2] [소비 시도]  ? <- [data2, data3]
17:09:05.800 [consumer2] [take] 소비자 데이터 획득, notify() 호출
17:09:05.800 [consumer2] [소비 완료] data2 <- [data3]
17:09:05.911 [consumer3] [소비 시도]  ? <- [data3]
17:09:05.911 [consumer3] [take] 소비자 데이터 획득, notify() 호출
17:09:05.911 [consumer3] [소비 완료] data3 <- []

17:09:06.022 [     main] 현재 상태 출력, 큐 데이터 : []
17:09:06.022 [     main] producer1: TERMINATED
17:09:06.022 [     main] producer2: TERMINATED
17:09:06.022 [     main] producer3: TERMINATED
17:09:06.022 [     main] consumer1: TERMINATED
17:09:06.022 [     main] consumer2: TERMINATED
17:09:06.025 [     main] consumer3: TERMINATED
17:09:06.025 [     main] == [생산자 먼저 실행] 종료, BoundedQueueV3 ==
```

왜 이런 결과가 나왔는지 보도록 하자. 먼저, BoundedMain에서 메인 스레드가 log 메소드를 실행하고 startProducer 메소드도 실행한다.
이제 for문으로 인해 3개의 스레드가 생성되고 2개의 스레드는 데이터를 정상적으로 넣는다.
세번째 스레드는 큐의 공간이 다 찼기 때문에 데이터를 넣지 못한다. 원래는 데이터를 버려야 했지만 wait(); 메소드 때문에 producer3 스레드는
락을 반납하고 스레드 대기 집합에서 WAITING 상태로 대기한다. 이제 큐에는 [data1, data2]가 있고 producer1,2는 TERMINATED 상태로 출력된다.
producer3 스레드는 계속 WAITING 상태를 유지한다. 이후에 소비자 스레드가 실행되는데 먼저 data1을 빼가고 notify(); 메소드를 호출해서 스레드 대기 집합에
있는 스레드를 깨운다. 이때 임의의 스레드를 깨운다. 일어난 producer3 스레드는 BLOCKED 상태로 있는다. 이러한 이유는 임계 영역에서 RUNNABLE 상태면
공통 자원을 건드리기 때문이다. consumer1 스레드는 락을 반납하고 producer3 스레드는 락을 획득하여 RUNNABLE 상태로 전환해서 본인의 일을 다하고나서 
락을 반납하고 consumer2, consumer3 스레드가 일을 끝마친다. 물론 notify(); 메소드를 호출해서 스레드 집합을 계속 확인하지만 대기하는 스레드가 없기에 아무 일도
안일어난다.

이제 반대로, 생산자를 먼저 실행하지 않고 소비자를 먼저 실행해보자. 

```text
18:00:12.761 [     main] == [소비자 먼저 실행] 시작, BoundedQueueV3 ==

18:00:12.767 [     main] 소비자 시작
18:00:12.781 [consumer1] [소비 시도]  ? <- []
18:00:12.781 [consumer1] [take] 큐에 데이터 없음, 소비자 대기
18:00:12.891 [consumer2] [소비 시도]  ? <- []
18:00:12.891 [consumer2] [take] 큐에 데이터 없음, 소비자 대기
18:00:13.002 [consumer3] [소비 시도]  ? <- []
18:00:13.006 [consumer3] [take] 큐에 데이터 없음, 소비자 대기

18:00:13.112 [     main] 현재 상태 출력, 큐 데이터 : []
18:00:13.117 [     main] consumer1: WAITING
18:00:13.123 [     main] consumer2: WAITING
18:00:13.123 [     main] consumer3: WAITING

18:00:13.123 [     main] 생산자 시작
18:00:13.123 [producer1] [생산 시도] data1 -> []
18:00:13.123 [producer1] [put] 생산자 데이터 저장, notify() 호출
18:00:13.128 [consumer1] [take] 소비자 깨어남
18:00:13.128 [consumer1] [take] 소비자 데이터 획득, notify() 호출
18:00:13.128 [consumer2] [take] 소비자 깨어남
18:00:13.128 [producer1] [생산 완료] data1 -> [data1]
18:00:13.128 [consumer1] [소비 완료] data1 <- []
18:00:13.130 [consumer2] [take] 큐에 데이터 없음, 소비자 대기
18:00:13.239 [producer2] [생산 시도] data2 -> []
18:00:13.239 [producer2] [put] 생산자 데이터 저장, notify() 호출
18:00:13.241 [consumer3] [take] 소비자 깨어남
18:00:13.241 [producer2] [생산 완료] data2 -> [data2]
18:00:13.241 [consumer3] [take] 소비자 데이터 획득, notify() 호출
18:00:13.243 [consumer3] [소비 완료] data2 <- []
18:00:13.243 [consumer2] [take] 소비자 깨어남
18:00:13.243 [consumer2] [take] 큐에 데이터 없음, 소비자 대기
18:00:13.350 [producer3] [생산 시도] data3 -> []
18:00:13.350 [producer3] [put] 생산자 데이터 저장, notify() 호출
18:00:13.350 [consumer2] [take] 소비자 깨어남
18:00:13.350 [producer3] [생산 완료] data3 -> [data3]
18:00:13.350 [consumer2] [take] 소비자 데이터 획득, notify() 호출
18:00:13.350 [consumer2] [소비 완료] data3 <- []

18:00:13.461 [     main] 현재 상태 출력, 큐 데이터 : []
18:00:13.461 [     main] consumer1: TERMINATED
18:00:13.461 [     main] consumer2: TERMINATED
18:00:13.464 [     main] consumer3: TERMINATED
18:00:13.464 [     main] producer1: TERMINATED
18:00:13.466 [     main] producer2: TERMINATED
18:00:13.466 [     main] producer3: TERMINATED
18:00:13.468 [     main] == [소비자 먼저 실행] 종료, BoundedQueueV3 ==
```

결과를 분석해보면 소비자를 먼저 실행했을 때는, 큐에 꺼낼 데이터가 없으니까 wait(); 으로 인해 락을 반납하고 대기집합에 머무른다. 
consumer2, consumer3 스레드도 마찬가지로 락을 반납하고 대기집합에 머무른다. (꺼낼 데이터가 없으므로)
상태를 출력할 때, 당연히 큐에 데이터는 없으므로 [] 가 출력되고 세 스레드는 다 대기집합에 있으므로 WAITING 상태를 유지한다.

생산자 스레드를 실행해보자. 생산자1 스레드는 먼저 큐에 data1을 저장하고 notify(); 를 호출했으니까 대기 집합에 있는 소비자 스레드를 깨운다.
이때 어떤 스레드가 깨워질지는 알 수 없다. consumer1 스레드는 깨어나고 producer1 스레드가 락을 반납한 뒤에 락을 얻고나서 큐에 있는 데이터를 빼간다. 
이때 notify(); 메소드를 또 호출하니까 consumer2나 consumer3 스레드를 깨운다. 일어난 스레드는 데이터가 없으니 다시 락을 반납한다. notify();를 호출하지 않는다.
이제 producer2 스레드가 락을 획득하고 큐에 데이터를 채운다. notify();로 인해 대기 집합에 있는 임의의 스레드를 깨우고나서 락을 반납하고 일어난 스레드가 락을 획득하고나서
데이터를 빼간다. 이러한 과정이 반복된다.

여기서 아쉬운 점은 대기 집합에있는 consumer 스레드를 깨웠는데 큐에 데이터가 없어서 다시 대기 집합으로 들어가니까 의미없게 CPU를 사용한다.

## 5. Object - wait, notify 한계

지금까지 살펴본 Object.wait(), Object.notify() 방식은 스레드 대기 집합 하나에 생산자, 소비자 스레드를 모두 관리한다.
그리고 notify()를 호출할 때 임의의 스레드가 선택된다. 따라서 앞서 살펴본 것 처럼 큐에 데이터가 없는 상황에 소비자가
같은 소비자를 깨우는 비효율이 발생할 수 있다. 또는 큐에 데이터가 가득 차있는데 생산자가 같은 생산자를 깨우는 비효율도 발생할 수 있다. 

다음과 같은 상황을 가정해보자.
예를 들어서, 스레드 대기 집합에 consumer1, consumer2, consumer3, producer1, producer2, producer3 스레드가 있다고 가정하자.
그리고 queue에는 dataX가 있다고 생각하자.

이제 producer0이라는 스레드가 락을 획득하여 data0를 queue에 넣었다고 생각해보자.
이러면 큐에 데이터가 가득 찬다. producer0 스레드는 notify 메소드를 통해 스레드 대기 집합에 있는 스레드를 깨운다. (단, 임의의 스레드를 깨운다.)
소비자 스레드를 깨우면 상관이 없지만 생산자 스레드를 깨우면 문제가된다. 안그래도 큐에 공간이 없는데 없는 상황에서 데이터를 넣으려니까 할 작업이 없어지기 때문이다.
만약에 깨어난 생산자 스레드가 또 다른 생산자 스레드를 깨우면 이러면 또 반복되는 상황이 발생한다. 

이번엔 다른 상황을 생각해보자. 만약에 consumer0이라는 스레드가 락을 획득하여 스레드 대기 집합에 있는 consumer1~5, producer1 스레드를 notify() 메소드로 인해서
깨워야 한다면 임의의 스레드가 깨어날 것이다. 근데 소비자 스레드만 계속 깨우면 대기 상태의 스레드(producer1)가 실행 순서를 계속 얻지 못해서 실행되지 않는 상황을 스레드 기아라 한다.
